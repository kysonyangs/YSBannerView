...
@interface YSHollowPageControl ()

@property (nonatomic, strong) NSMutableArray *dots; /**< 保存所有的点 */

@end

@implementation YSHollowPageControl
...
- (void)_initializationSetting{
    self.dotViewClass           = [YSAnimatedDotView class];
...
- (void)touchesBegan:(NSSet *)touches withEvent:(UIEvent *)event{
    ...
    if (touch.view != self) {
        ...
        if ([self.delegate respondsToSelector:@selector(hollowPageControl:didSelectPageAtIndex:)]) {
            [self.delegate hollowPageControl:self didSelectPageAtIndex:index];
        }
    }
}
...
- (UIView *)generateDotView{
    ...
    if (self.dotViewClass) {
        ...
        if ([dotView isKindOfClass:[YSAnimatedDotView class]]) {
            if (self.resizeScale > 0) {
                ((YSAnimatedDotView *)dotView).resizeScale = self.resizeScale;
            }
            if (self.dotNormalColor) {
                ((YSAnimatedDotView *)dotView).dotColor = self.dotNormalColor;
            }
            if (self.dotCurrentColor){
                ((YSAnimatedDotView *)dotView).currentDotColor = self.dotCurrentColor;
            }
        }
    } else {
...
- (void)changeActivity:(BOOL)active atIndex:(NSInteger)index{
    if (self.dotViewClass) {
        YSAbstractDotView *abstractDotView = (YSAbstractDotView *)[self.dots objectAtIndex:index];
        if ([abstractDotView respondsToSelector:@selector(changeActivityState:)]) {
            [abstractDotView changeActivityState:active];
        } else {
            NSLog(@"Custom view : %@ must implement an 'changeActivityState' method or you can subclass %@ to help you.", self.dotViewClass, [YSAbstractDotView class]);
        }
